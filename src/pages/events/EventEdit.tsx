...
const EventEdit: React.FC = () => {
  ...
  const [totalSteps] = useState(5); // Agregamos paso para resultados si está completado
    ...
    const loadEventData = async () => {
      ...
      try {
        ...
        const mockEventData: EventFormData = {
          ...
          bovineIds: ["bov_001", "bov_002", "bov_003"],
          ...
          results: {
            ...
            medicationsAdministered: ["Vacuna Fiebre Aftosa"],
            dosages: {
        ...
        setOriginalData(mockEventData);
        setFormData(mockEventData);
        setBovineOptions(mockBovines);
        setVeterinarianOptions(mockVeterinarians);

        // Determinar si mostrar paso de resultados
        setShowResultsStep(mockEventData.status === "completed");
      } catch (error) {
  ...
  useEffect(() => {
    if (originalData && formData) {
      const changes = JSON.stringify(originalData) !== JSON.stringify(formData);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      console.log("Datos actualizados del evento:", formData);

      // Mostrar éxito y navegar de vuelta
      alert("Evento actualizado exitosamente");
      navigate(`/events/detail/${formData.id}`);
    } catch (error) {
      console.error("Error actualizando evento:", error);
      alert("Error al actualizar el evento. Inténtalo nuevamente.");
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleStatusChange = (newStatus: string) => {
    ...
    if (newStatus === "completed" && !formData.completedAt) {
      handleInputChange("completedAt", new Date().toISOString());
      setShowResultsStep(true);
    }

    // Si se desmarca como completado, quitar timestamp y resultados
    if (newStatus !== "completed" && formData.completedAt) {
      handleInputChange("completedAt", undefined);
      handleInputChange("results", undefined);
      setShowResultsStep(false);
    }
  };

  // Obtener el tipo de evento seleccionado
  const selectedEventType = formData
    ? eventTypes.find((type) => type.id === formData.eventType)
    : null;
  ...
  const renderCustomFields = () => {
    if (!selectedEventType || !formData) return null;
  ...
  const renderResultsStep = () => {
    if (!formData || !formData.results) return null;
      ...
      <div className="space-y-6">
        <div className="text-center mb-8">
          ...
          <p className="text-gray-600">
            Registra los resultados y seguimiento del evento completado
          </p>
        </div>
        ...
        <div className="space-y-4">
          <label className="block text-sm font-medium text-gray-700">
            Estado del Resultado <span className="text-red-500">*</span>
          </label>
          <div className="flex space-x-4">
            <motion.button
              whileHover={{ scale: 1.02 }}
              whileTap={{ scale: 0.98 }}
              onClick={() => handleResultsChange("success", true)}
            ...
            >
              ...
              <p className="font-medium">Exitoso</p>
            </motion.button>

            <motion.button
              whileHover={{ scale: 1.02 }}
              whileTap={{ scale: 0.98 }}
              onClick={() => handleResultsChange("success", false)}
              className={`flex-1 p-4 border-2 rounded-xl transition-all ${
                !results.success
            ...
            >
              ...
              <p className="font-medium">Con Complicaciones</p>
            </motion.button>
          </div>
        </div>
        ...
        <div className="space-y-2">
          <label className="block text-sm font-medium text-gray-700">
            Notas del Resultado <span className="text-red-500">*</span>
          </label>
          <textarea
            value={results.notes || ""}
            onChange={(e) => handleResultsChange("notes", e.target.value)}
            placeholder="Describe los resultados del evento..."
            rows={4}
        ...
        {/* Complicaciones (si no fue exitoso) */}
        {!results.success && (
          <div className="space-y-2">
            <label className="block text-sm font-medium text-gray-700">
              Complicaciones
            ...
            <textarea
              ...
              onChange={(e) =>
                handleResultsChange("complications", e.target.value)
              }
              placeholder="Describe las complicaciones presentadas..."
              rows={3}
              className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent bg-white/80 resize-none"
            />
          </div>
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-2">
              ...
              <input
                type="number"
                value={results.vitals?.temperature || ""}
                onChange={(e) =>
                  handleResultsChange("vitals", {
                    ...results.vitals,
            ...
            <div className="space-y-2">
              ...
              <input
                type="number"
                value={results.vitals?.heartRate || ""}
                onChange={(e) =>
                  handleResultsChange("vitals", {
                    ...results.vitals,
            ...
            <div className="space-y-2">
              ...
              <input
                type="number"
                value={results.vitals?.respiratoryRate || ""}
                onChange={(e) =>
                  handleResultsChange("vitals", {
                    ...results.vitals,
        ...
        <div className="space-y-4">
          <div className="flex items-center space-x-2">
            <input
              type="checkbox"
              id="followUpRequired"
              checked={results.followUpRequired}
              onChange={(e) =>
                handleResultsChange("followUpRequired", e.target.checked)
              }
          ...
          {results.followUpRequired && (
            <div className="pl-6 space-y-4 border-l-2 border-blue-200">
              <div className="space-y-2">
                ...
                <input
                  type="date"
                  value={results.followUpDate || ""}
                  onChange={(e) =>
                    handleResultsChange("followUpDate", e.target.value)
                  }
  ...
  const renderStep = () => {
    ...
    switch (currentStep) {
      case 1:
        return (
          <div className="space-y-6">
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3">
                {[
                  ...
                  { value: "postponed", label: "Pospuesto", color: "purple" },
                ].map((status) => (
                  <motion.button
                    key={status.value}
                    whileHover={{ scale: 1.02 }}
            ...
            {/* Tipo de Evento (Solo lectura en edición) */}
            <div className="space-y-4">
              <label className="block text-sm font-medium text-gray-700">
                Tipo de Evento
              </label>
              {selectedEventType && (
                <div className="p-4 border-2 border-gray-200 rounded-xl bg-gray-50">
                  <div className="flex items-center space-x-3">
                    <div
                      className={`p-2 rounded-lg ${selectedEventType.color} text-white`}
                    >
                  ...
                  <p className="text-xs text-gray-500 mt-2">
                    El tipo de evento no puede modificarse después de la
                    creación
                  </p>
                </div>
              )}
        ...
        return (
          <div className="space-y-6">
            <div className="text-center mb-8">
              ...
              <p className="text-gray-600">
                Modifica cuándo y dónde se realizará el evento
              </p>
            </div>
        ...
        return (
          <div className="space-y-6">
            <div className="text-center mb-8">
              ...
              <p className="text-gray-600">
                Modifica los animales y personal involucrado
              </p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-2">
                ...
                <input
                  ...
                  onChange={(e) =>
                    handleInputChange("actualCost", Number(e.target.value))
                  }
                  placeholder="0.00"
                  min="0"
                  step="0.01"
        ...
        return (
          <div className="space-y-6">
            <div className="text-center mb-8">
              ...
              <p className="text-gray-600">
                Modifica la información específica del evento
              </p>
            </div>
      ...
      case 5:
        return showResultsStep ? renderResultsStep() : null;
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-green-50 flex items-center justify-center">
        ...
        >
          ...
          <p className="text-gray-600 text-lg">Cargando datos del evento...</p>
        </motion.div>
      </div>
    );
  }
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              <motion.button
                whileHover={{ scale: 1.05 }}
                whileTap={{ scale: 0.95 }}
                onClick={() => navigate(`/events/detail/${formData.id}`)}
              ...
              <div>
                ...
                <div className="flex items-center space-x-4 mt-1">
                  <p className="text-gray-600">
                    Paso {currentStep} de{" "}
                    {showResultsStep ? totalSteps : totalSteps - 1} -{" "}
                    {currentStep === 1
                      ? "Información Básica"
                      : currentStep === 2
                      ? "Fecha y Ubicación"
            ...
            <div className="flex items-center space-x-4">
              ...
              <div className="flex items-center space-x-2">
                {Array.from(
                  { length: showResultsStep ? totalSteps : totalSteps - 1 },
                  (_, i) => (
                    <div
                      key={i}
      ...
      >
        <div className="bg-white/80 backdrop-blur-sm rounded-2xl border border-gray-200 p-8">
              ...
              <div className="flex items-center space-x-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-yellow-700">
                    Has realizado modificaciones que no se han guardado aún.
                  </p>
                </div>
                <motion.button
                  whileHover={{ scale: 1.05 }}
          ...
          <div className="flex items-center justify-between mt-8 pt-6 border-t border-gray-200">
            ...
            <div className="flex items-center space-x-3">
              {currentStep < (showResultsStep ? totalSteps : totalSteps - 1) ? (
                <motion.button
                  whileHover={{ scale: 1.05 }}
                  whileTap={{ scale: 0.95 }}
                  onClick={() =>
                ...
                <motion.button
                  ...
                  className="flex items-center space-x-2 px-8 py-3 bg-gradient-to-r from-green-600 to-blue-600 text-white rounded-xl hover:from-green-700 hover:to-blue-700 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  {saving ? (
                    <div className="animate-spin rounded-full h-5 w-5 border-b-2 border-white"></div>
                  ) : (
